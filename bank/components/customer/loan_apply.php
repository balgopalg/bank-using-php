...
<body>
    ...
    <main>
        ...
        if(isset($_POST['apply'])){
            $accno=$_POST['accno'];
            $checkStatus="SELECT * FROM loanapp WHERE accno='$accno' AND status='INPROCESS';";
            $result=mysqli_query($conn,$checkStatus);
            if(mysqli_num_rows($result)>0){
                echo "<script>alert('you have ongoing loan appplication you can't apply more.');</script>";
            }else{
                $name=$_POST['name'];
                $reason=$_POST['reason'];
